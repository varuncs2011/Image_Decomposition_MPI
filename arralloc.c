...
// This function allocates memory of length*width*sizeofdouble elements
// The base address of the every vertical stride of the allocated memory is stored in 
// array of pointers.
 
void allocatememory(double ***arr, double **content, int length, int width)
{

	*arr = (double **)malloc(length * sizeof(double *));

	*content = (double *)malloc(length * width * sizeof(double));

	
	for (int i = 0; i != length; ++i)
	{
	  // for a rectangular grid of M*N which is divided vertically into N grids, arr[i] will store the base address of i th grid.
		(*arr)[i] = *content + width * i;
	}
}


// Free memory pointed by the array of pointers and 2D array
void releasememory(double ***arr, double **content)
{
	if (NULL != content)
	{
		free(*content);
