...
/*
 *  This method will read the chunk of PGM file in 2D array 
 
*/

void pgmreadchunk(char *filename, void *vx, int nx, int ny, int start_x, int start_y, int row)
{
  FILE *fp;
  ...
  for (j = 0; j < ny; j++)
  {
    ...
    {
      iret = fscanf(fp, "%d", &t);
     // printf("value of first: %d ,value of i =%d \n",( (ny - j - 1) + ny * i),i);
      x[(ny - j - 1) + ny * i + i * row] = t;
    }
